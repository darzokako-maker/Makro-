...
class YahyaUltimateMacro(ctk.CTk):
    def __init__(self):
        ...
        # G Hub / G300 tarzı özel makro tablosu (ayrık Aşağı/Yukarı adımları + saniye cinsinden gecikme)
        self.macro_steps = []       # [{"action": "left_down"/"left_up"/.../"delay", "value": sn|tuş|None}, ...]
        self.macro_key = "m"
        ...
        ctk.CTkLabel(self.custom_macro_frame, text="ÖZEL MAKRO (G Hub Tarzı Adım Tablosu)", font=("Arial", 12, "bold"), text_color="#00FF7F").pack(pady=5)
    ...
    def open_macro_editor(self):
        """G Hub / G300 tarzı: her tık ayrı Aşağı/Yukarı adımı, aralarda saniye cinsinden (eksili olabilen) gecikme."""
        self.macro_win = ctk.CTkToplevel(self)
        self.macro_win.title("Özel Makro Düzenleyici (G Hub Tarzı)")
        self.macro_win.geometry("500x600")
        self.macro_win.attributes("-topmost", True)

        ctk.CTkLabel(self.macro_win, text="MAKRO ADIMLARI", font=("Arial", 16, "bold")).pack(pady=10)
        ctk.CTkLabel(
            self.macro_win,
            text="Her düğme için ayrı 'Aşağı' ve 'Yukarı' satırı ekle.\n"
                 "Aralarına 'Gecikme (sn)' satırı koy — saniye cinsinden,\n"
                 "ondalıklı (0.5) ve eksili (-0.2) değerler kabul edilir.",
            font=("Arial", 11), justify="left"
        ).pack(pady=(0, 10))

        self.macro_scroll = ctk.CTkScrollableFrame(self.macro_win, width=440, height=380)
        self.macro_scroll.pack(pady=5, padx=10, fill="both", expand=True)

        self.macro_row_widgets = []
        if self.macro_steps:
            code_to_label = {v: k for k, v in self.MACRO_ACTION_MAP.items()}
            for step in self.macro_steps:
                label = code_to_label.get(step["action"], "Gecikme (sn)")
                raw = "" if step["value"] is None else str(step["value"])
                self.add_macro_row(action_label=label, value=raw)
        else:
            self.add_macro_row()
    ...
    # Etiket -> iç kod eşlemesi (tek merkezden yönetiliyor, editör ve kaydetme aynısını kullanıyor)
    MACRO_ACTION_MAP = {
        "Sol Düğme Aşağı": "left_down",   "Sol Düğme Yukarı": "left_up",
        "Sağ Düğme Aşağı": "right_down",  "Sağ Düğme Yukarı": "right_up",
        "Orta Düğme Aşağı": "middle_down", "Orta Düğme Yukarı": "middle_up",
        "X1 Düğme Aşağı": "x1_down",      "X1 Düğme Yukarı": "x1_up",
        "X2 Düğme Aşağı": "x2_down",      "X2 Düğme Yukarı": "x2_up",
        "Tuş Aşağı": "key_down",          "Tuş Yukarı": "key_up",
        "Gecikme (sn)": "delay",
    }

    def add_macro_row(self, action_label="Sol Düğme Aşağı", value=""):
        ...
        action_menu = ctk.CTkOptionMenu(row_frame, values=list(self.MACRO_ACTION_MAP.keys()), variable=action_var, width=160)
        action_menu.pack(side="left", padx=5)

        value_entry = ctk.CTkEntry(row_frame, width=80, placeholder_text="sn / tuş")
    ...
    def save_macro_steps(self):
        ...
        for row in self.macro_row_widgets:
            ...
            action = self.MACRO_ACTION_MAP.get(action_label, "delay")
            ...
            if action in ("key_down", "key_up"):
                ...
                try:
                    value = float(raw_value.replace(",", "."))  # "0,5" veya "0.5" ikisi de kabul, eksili olabilir
                except ValueError:
                    value = 0.0
            else:
                value = None  # düğme aşağı/yukarı adımlarında süre gerekmiyor
    ...
    def execute_macro_step(self, step):
        ...
        if action == "left_down":
            ...
            # Saniye cinsinden; eksili yazılabilir ama fiili bekleme negatif olamayacağı için burada 0'a çekilir.
